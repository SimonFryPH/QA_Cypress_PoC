...
cy.config().screenSizes.forEach((size) => {
  ...
  describe('Our Holiday Parks ' + size[0] + ', ' + size[1], async function () {
    ...
    it('Should display correct holiday & short break parks', function () {
      ...
      cy.get('#root').then(() => {
        ...
        if (el.outerWidth() < 768) {
          cy.get('.button--red').eq(0).click() // expand menu first
        }
      })
      ...
      cy.get('#root').then(() => {
        let el = Cypress.$('#root') 
        if (el.outerWidth() < 768) {
          cy.get('.button--red').eq(1).click() // collapse menu
        }
      })
...
/
      ...
      cy.get(".col-xs-12:not([style='display: none;']):nth-child(" + i + ") .title p").then($parkName => {
        ...
        cy.get('h3 span').eq(0).should('include.text', "Come and see")
        cy.get('._r3h2fp').eq(0).should('include.text', parkNameText)
        ...
        cy.log("XXXXXXXXXXXXXXXXXXXXXXXXXXXXX")
