    ...
    it('Should Search Holiday homes (ownership)', function () {
      ...
      cy.get('#root').then(() => {
        ...
        if (el.outerWidth() < 768) {
          cy.get('.button--red').eq(1).scrollIntoView().click() // expand menu first
        }
      })

      cy.get('#filter').should('be.visible')
      cy.get('#filter > div.filters__options > div:nth-child(3)').scrollIntoView().should('be.visible') // Condition
      cy.get('#filter > div.filters__options > div:nth-child(4)').scrollIntoView().should('be.visible') // Type
      cy.get('#filter > div.filters__options > div:nth-child(6)').scrollIntoView().should('be.visible') // Bedrooms
      cy.get('#filter > div.filters__options > div:nth-child(8)').scrollIntoView().should('be.visible') // Addional Options


      cy.get('#filter > div.filters__options > div:nth-child(9)').scrollIntoView().should('be.visible') // Manufacturer
      cy.get('#filter > div.filters__options > div:nth-child(9) > a').scrollIntoView().should('be.visible') // Manufacturer Show all options
      ...
      cy.get('#root').then(() => {
        let el = Cypress.$('#root') 
        if (el.outerWidth() < 768) {
          cy.get('.button--red').eq(0).scrollIntoView().click() // collapse menu
        }
      })


      cy.log("* Known Bug  https://parkholidaysuk.atlassian.net/browse/WEB-927")
